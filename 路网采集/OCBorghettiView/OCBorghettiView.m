...
- (void)addSectionWithTitle:(NSString *)sectionTitle andView:(id)sectionView andImage:(NSString *)imgStr andName:(NSString *)nameStr andStake:(NSString *)stakeStr andHuman:(NSString *)humanStr andDate:(NSString *)dateStr andStakeEnd:(NSString *)stakeEndStr andContent:(NSString *)contentStr
{
    ...
    UIImageView *photoIv = [[UIImageView alloc]initWithFrame:CGRectMake(10, 10, 57, 57)];
    photoIv.image = [UIImage imageNamed:imgStr];
    [section addSubview:photoIv];
    
    
    NSArray *array = [NSArray arrayWithObjects:nameStr,[NSString stringWithFormat:@"中心桩号:%@",stakeStr],[NSString stringWithFormat:@"采集人员:%@",humanStr],[NSString stringWithFormat:@"采集时间:%@",dateStr], nil];
    ...
    if([contentStr isEqualToString:@"点状数据"])
    {
        ...
        for(int i=0;i<4;i++)
        {
            ...
            if(i<3)
            {
                ...
                nameLabel = [[UILabel alloc]initWithFrame:CGRectMake(70, 10+20*i, 200, 20)];
                if(i==0)
                {
            ...
            else
            {
                ...
                nameLabel = [[UILabel alloc]initWithFrame:CGRectMake(70+130, 10+20*2, 120, 20)];
                nameLabel.adjustsFontSizeToFitWidth = YES;
                nameLabel.textAlignment = NSTextAlignmentRight;
                
            }
            
            nameLabel.text = [array objectAtIndex:i];
    ...
    if([contentStr isEqualToString:@"线状数据"])
    {
        ...
        array = [NSArray arrayWithObjects:nameStr,[NSString stringWithFormat:@"起点桩号:%@",stakeStr],[NSString stringWithFormat:@"止点桩号:%@",stakeEndStr],[NSString stringWithFormat:@"采集人员:%@",humanStr],[NSString stringWithFormat:@"采集时间:%@",dateStr], nil];
        for(int i=0;i<5;i++)
        {
            ...
            if(i<4)
            {
                ...
                nameLabel = [[UILabel alloc]initWithFrame:CGRectMake(70, 20*i, 200, 20)];
                if(i == 0)
                {
            ...
            else
            {
                ...
                nameLabel = [[UILabel alloc]initWithFrame:CGRectMake(70+130, 20*3, 120, 20)];
                nameLabel.adjustsFontSizeToFitWidth = YES;
            }
...
- (void)layoutSubviews
{
    ...
    for (int i = 0; i < self.views.count; i++) {
        ...
        if (self.accordionSectionActive == i) {
            [sectionTitle setImage:[UIImage imageNamed:@"上回箭头蓝"]
                          forState:UIControlStateNormal];
            ...
            [sectionTitle setImage:[UIImage imageNamed:@"下拉箭头灰"]
                          forState:UIControlStateNormal];
