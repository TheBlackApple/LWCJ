//
//  OfflineProvinceViewController.m
//  路网采集
//
...
@interface OfflineProvinceViewController ()<BMKMapViewDelegate,BMKOfflineMapDelegate,UITableViewDataSource,UITableViewDelegate,OfflineCellDelegate>
{
    BMKMapView * _mapView;
...
@implementation OfflineProvinceViewController
...
- (void)initView
{
    ...
    if (city.count > 0) {
        ...
        self.count = oneCity.childCities.count - 1;
        _arrayOfflineCityData =[NSMutableArray arrayWithObject:[oneCity.childCities objectAtIndex:0]];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSNumber * localmapid = [userDef objectForKey:@"province"];
    if ([localmapid integerValue] == 1) {
    ...
    if (type == TYPE_OFFLINE_UPDATE) {
        ...
        if (updateInfo.ratio == 100)
        {
            ...
            if (self.count == 0) {
                ...
                [[NSUserDefaults standardUserDefaults]setObject:[NSNumber numberWithBool:YES] forKey:@"province"];
                [[NSUserDefaults standardUserDefaults]synchronize];
            }
            else
            {
                self.myCell.mRadio.text = [NSString stringWithFormat:@"下载%@",updateInfo.cityName];
            }
        }
    }
